...
class Pendaftaran extends Page implements HasForms
{
    ...
    public function detailTransaction($result)
    {
        ...
        $registrasi = \App\Models\Pendaftaran::with('peserta')->where('uuid_pendaftaran', $transactionOrderId)->first();
        ...
        // Update pembayaran, peserta and registrasi
        $this->updatePayment(
            $pembayaran,
            $transactionOrderId,
            $paymentTypeEnum,
    ...
    private static function buildDataPesertaFields(): array
    {
        return [
            ...
            Forms\Components\TextInput::make('peserta.nama_lengkap')
                ->label('Nama Peserta')
                ->required()
                ->autofocus()
                ->maxLength(255),
            Forms\Components\TextInput::make('peserta.no_telp')
                ->label('Nomor Telepon/WA Peserta')
                ->required()
                ->numeric()
                ->maxLength(12),
            Forms\Components\TextInput::make('peserta.email')
                ->email()
                ->unique(ignoreRecord: true)
                ->label('Email Peserta')
                ->required()
                ->maxLength(255),
            Forms\Components\TextInput::make('peserta.tempat_lahir')
                ->label('Tempat Lahir')
                ->required(),
            Forms\Components\DatePicker::make('peserta.tanggal_lahir')
                ->label('Tanggal Lahir')
                ->required()
                ->displayFormat('d-m-Y')
                ->format('Y-m-d')
                ->rules([fn(): Closure => self::validateMinimumAgeClosure()]),
            Forms\Components\Select::make('peserta.jenis_kelamin')
                ->label('Jenis Kelamin')
                ->options(JenisKelamin::class)
                ->required()
                ->default(JenisKelamin::LAKI),
            Forms\Components\Select::make('peserta.tipe_kartu_identitas')
                ->label('Tipe Kartu Identitas')
                ->options(TipeKartuIdentitas::class)
                ->required()
                ->default(TipeKartuIdentitas::KTP),
            Forms\Components\TextInput::make('peserta.nomor_kartu_identitas')
                ->label('Nomor Kartu Identitas')
                ->required()
                ->numeric(),
            Forms\Components\TextInput::make('peserta.nama_kontak_darurat')
                ->label('Nama Kontak Darurat')
                ->required(),
            Forms\Components\TextInput::make('peserta.nomor_kontak_darurat')
                ->label('Nomor Kontak Darurat')
                ->required(),
            Forms\Components\Select::make('peserta.golongan_darah')
                ->label('Golongan Darah')
                ->options(GolonganDarah::class)
                ->required(),
            Forms\Components\TextInput::make('peserta.komunitas')
    ...
    private static function stepDataAlamatPeserta(): Forms\Components\Wizard\Step
    {
        return Forms\Components\Wizard\Step::make('Data Alamat Peserta')
            ...
            ->completedIcon('heroicon-m-map-pin')
            ->schema([
    ...
    private static function stepDataPendaftaranPeserta(): Forms\Components\Wizard\Step
    {
        return Forms\Components\Wizard\Step::make('Data Pendaftaran')
            ...
            ->completedIcon('heroicon-m-credit-card')
            ->schema([
                Section::make('Data Pendaftaran Peserta')
